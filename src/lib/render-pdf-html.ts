...
export function renderPdfHtml(scan: ScanResponse, lang: Locale): string {
  ...
  const checksHtml = scan.checks
    .map((c) => {
      ...
      const label = lang === 'es' && c.labelEs ? c.labelEs : c.label;
      const details = lang === 'es' && c.detailsEs ? c.detailsEs : c.details;
      const suggestion = lang === 'es' && c.suggestionEs ? c.suggestionEs : c.suggestion;
      return `
        <tr>
          <td>${label}</td>
  ...
  const recommendations = scan.checks
    ...
    .map((c) => {
      const label = lang === 'es' && c.labelEs ? c.labelEs : c.label;
      const suggestion = lang === 'es' && c.suggestionEs ? c.suggestionEs : c.suggestion;
      return `<li><strong>${label}:</strong> ${suggestion}</li>`;
    })
    .join('');
